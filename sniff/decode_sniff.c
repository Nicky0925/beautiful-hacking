...
#include "../socket/hacking-network.h"
...
void decode_ethernet(const u_char *header_start) {
    ...
    printf("[[第２層::イーサネットヘッダ]]\n");
    printf("[送信元: %02x", ethernet_header->ehter_src_addr[0]);
    for (i = 1; i < ETHER_ADDR_LEN; i++) {
        printf(":%02x", ethernet_header->ehter_src_addr[i]);
    }
