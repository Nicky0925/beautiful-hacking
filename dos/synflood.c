...
int main(int argc, char *argv[]) {
  ...
  int packet_size = LIBNET_TCP_H + LIBNET_IP_H; //LIBNET_IP_H + LIBNET_TCP_H
  ...
  if (network == -1) {
    libnet_error(LIBNET_ERR_FATAL, "ネットワークインタフェースを開くことができません。--このプログラムはrootで実行しなければいけません。\n");
  }
  libnet_init_packet(packet_size, &packet);
  if (packet == NULL) {
    libnet_error(LIBNET_ERR_FATAL, "メモリの割り当てができません。\n");
  }
  libnet_seed_prand();
  ...
  while(1) {
    ...
    if (libnet_do_checksum(packet, IPPROTO_TCP, LIBNET_TCP_H) == -1) {
      libnet_error(LIBNET_ERR_FATAL, "チェックサムの計算ができません。\n");
    }
    ...
    usleep(FLOOD_DELAY); // 5000ms
  }

  libnet_destroy_packet(&packet);
  if (libnet_close_raw_sock(network) == -1) {
    libnet_error(LIBNET_ERR_WARNING, "cant't close network interface.");
  }
